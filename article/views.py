...
class ArticleView(APIView):
    ...
    def post(self, request):
        ...
        output_img = magic(
            filestr=request.FILES["input"].read(), style=request.data.get("style", "")
        )
...
class CommentView(APIView):
    ...
    def delete(self, request, comment_id):
        ...
        if comment.user.id == user:
            ...
            return Response(
                {"message": "댓글 작성자가 아닙니다"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class BookMarkView(APIView):
    ...
    def post(self, request):
        ...
        if existed_bookmark:
            ...
            return Response(
                {"message": "북마크가 취소 되었습니다."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class LikeView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if existed_like:
            existed_like.delete()
            return Response(
                {"message": "이미 좋아요 했슈"}, status=status.HTTP_400_BAD_REQUEST
            )
